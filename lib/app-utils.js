...
APP.setupControls = function() {
	...
	// APP.addControls(main, true, true); 					Removed for final Project
...
APP.addControls = function(effect, open, skipBase) {
	...
	if (!skipBase) {
		var base = {
			// Toggle:APP.Effect.toggle.bind(effect), 		Removed for final Project
			// Freeze:APP.Effect.toggleLoop,				Removed for final Project
			// Save:APP.Effect.save							Removed for final Project
		};
		for (var key in base) 
			f.add(base, key);
	}
